...
let movesCount = 0,
 ...
 const formatTime = (timer) => {
  let bestMinutes = Math.floor(timer / 60);
  let bestSeconds = timer % 60;
  let formattedTime = `${bestMinutes < 10? `0${bestMinutes}`: bestMinutes}:${bestSeconds < 10 ? `0${bestSeconds}` : bestSeconds}`;
        ...
        if (!firstCard) {
          ...
          if (firstCardValue == secondCardValue) {
            ...
            if (winCount == Math.floor(cardValues.length / 2)) {
                ...
                <h4>Time : ${formattedCurrentTime}</h4>
                <h4>Best Time : ${formattedBestTime}</h4>`;
